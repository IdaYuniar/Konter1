...
if(isset($_POST['update_modal'])){
    ...
    mysqli_query($conn, "UPDATE penjualan_offline SET modal='$modal', hasil=(jumlah-$modal) WHERE id_toko='$id_toko' AND kategori='$kategori' AND nama_produk='$nama'");
...
<div class="container py-4">
    ...
    <div class="card"><div class="card-body">
        ...
        <table class="table table-hover">
            <thead><tr><th>Tanggal</th><th>Hasil Keseluruhan</th><th>Pengeluaran</th><th>Hasil Bersih</th></tr></thead>
            <tbody>
                <?php
...
document.addEventListener('submit', function(e){
    if(e.target.classList.contains('modalForm')){
        e.preventDefault();
        const f = e.target;
        fetch(window.location.href, {method:'POST', body:new FormData(f)})
        .then(() => {
            const tr = f.closest('tr');
            const omset = parseFloat(tr.dataset.omset);
            const modal = parseFloat(f.modal.value) || 0;
            const hasilBaru = omset - modal;
            tr.querySelector('.hasilCell').innerText = 'Rp ' + hasilBaru.toLocaleString('id-ID');
            // Catatan: Jika ingin hasil bersih di tabel bawah ikut berubah total secara instan, 
            // disarankan untuk memuat ulang halaman atau menggunakan fetch data JSON agar sinkron dengan database.
